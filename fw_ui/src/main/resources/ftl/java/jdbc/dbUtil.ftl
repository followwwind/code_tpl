...
public class DbUtil {
     ...
     * @return
     */
    public static <T> Page<T> pageQuery(String sql, Consumer<PreparedStatement> psCall, Class<T> c, int pageNumber, int pageSize){
        if(pageNumber <= 0){
            pageNumber = 1;
        }
        String pageSql = String.format("select * from (%s) a limit %d, %d", sql, pageNumber - 1, pageSize);
        Page<T> page = new Page<>();
        page.setPageNumber(pageNumber);
        page.setLineNumber(pageSize);
        page.setData(executeQuery(pageSql, psCall, c));
        page.setTotalCount(count(sql, psCall));
        return page;
    }
     ...
     * @return
     */
    public static <T> List<T> executeQuery(String sql, Consumer<PreparedStatement> psCall, Class<T> c){
     ...
     * @return
     */
    public static <T> T parseTable(ResultSet rs, Class<T> c, Map<String, String> colMap) {
        T entity = null;
        try {
            ...
            Arrays.stream(fields).filter(f -> colList.contains(f.getName())).forEach(f -> {
                ...
                try {
                    ...
                    if(val != null){
                        if(type == Integer.class && val instanceof Boolean){
